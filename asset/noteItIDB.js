...
window.onload = function() {
    ...
    // Let us open our database, passing DB name and its version
    // Version should always be an integer and not any float value
    var DBOpenRequest = window.indexedDB.open("noteItDb", 1);
    ...
    DBOpenRequest.onsuccess = function(event) {
        console.log("DBOpenRequest successfull");
        // store the result of opening the database in the db variable. This is used a lot below
        dbData = DBOpenRequest.result;
    ...
    DBOpenRequest.onupgradeneeded = function(event) {
        ...
        db.onerror = function(event) {
            console.log("Error loading noteItDb");
        };

        // Create an noteItStore for this database

        var noteItStore = db.createnoteItStore("noteItDb", { keyPath: "noteId" });

        // define what data items the noteItStore will contain

        noteItStore.createIndex("noteUrl", "noteUrl", { unique: false });
        noteItStore.createIndex("timestamp", "timestamp", { unique: true });
        noteItStore.createIndex("noteContent", "noteContent", { unique: false });
        noteItStore.createIndex("theme", "theme", { unique: false });
        console.log("NoteItStore created!");
    };
}
...
window.onload = function() {
    ...
    DBOpenRequest.onupgradeneeded = function(event) {
        ...
        var noteItStore = db.createnoteItStore("toDoList", { keyPath: "taskTitle" });

        // define what data items the noteItStore will contain

        noteItStore.createIndex("hours", "hours", { unique: false });
        noteItStore.createIndex("minutes", "minutes", { unique: false });
        noteItStore.createIndex("day", "day", { unique: false });
        noteItStore.createIndex("month", "month", { unique: false });
        noteItStore.createIndex("year", "year", { unique: false });

        noteItStore.createIndex("notified", "notified", { unique: false });
    ...
    function displayData() {
        ...
        var noteItStore = db.transaction('toDoList').noteItStore('toDoList');
        noteItStore.openCursor().onsuccess = function(event) {
    ...
    function addData(e) {
        ...
        if (title.value == '' || hours.value == null || minutes.value == null || day.value == '' || month.value == '' || year.value == null) {
            ...
            var noteItStore = transaction.noteItStore("toDoList");
            console.log(noteItStore.indexNames);
            console.log(noteItStore.keyPath);
            console.log(noteItStore.name);
            console.log(noteItStore.transaction);
            console.log(noteItStore.autoIncrement);

            // add our newItem object to the object store
            var noteItStoreRequest = noteItStore.add(newItem[0]);
            noteItStoreRequest.onsuccess = function(event) {
    ...
    function deleteItem(event) {
        ...
        var transaction = db.transaction(["toDoList"], "readwrite");
        var request = transaction.noteItStore("toDoList").delete(dataTask);
    ...
    function checkDeadlines() {
        ...
        var noteItStore = db.transaction(['toDoList'], "readwrite").noteItStore('toDoList');
        noteItStore.openCursor().onsuccess = function(event) {
            var cursor = event.target.result;
            if (cursor) {
    ...
    function createNotification(title) {
        ...
        var noteItStoreTitleRequest = noteItStore.get(title);

        noteItStoreTitleRequest.onsuccess = function() {
            // grab the data object returned as the result
            var data = noteItStoreTitleRequest.result;

            // update the notified value in the object to "yes"
            data.notified = "yes";

            // create another request that inserts the item back into the database
            var updateTitleRequest = noteItStore.put(data);
